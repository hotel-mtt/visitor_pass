            ...
            for i, sess in enumerate(SESSIONS):
                ...
                with cols[i % 2]:
                    if is_taken:
                        ...
                        status_txt = "\u2713 Dipilih" if is_picked else "Tersedia"
                        sess_label = sess["label"]
                        st.markdown(
                            f'<div style="{ps}border-radius:8px;padding:10px 13px;margin-bottom:8px;">'
                            f'<div style="font-size:11px;font-weight:700;color:{lc};text-transform:uppercase;margin-bottom:4px;">{status_txt}</div>'
                            f'<div style="font-size:14px;font-weight:700;color:{tc};">{sess_label}</div></div>',
                            unsafe_allow_html=True)
